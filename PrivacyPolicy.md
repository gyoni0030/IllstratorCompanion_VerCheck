# プライバシーポリシー
最終更新日: 2026/03/19

本アプリ（以下、「本アプリ」）は、ユーザーのプライバシーを尊重し、以下の方針に基づいて情報を取り扱います。

■ 1. 収集する情報
本アプリは、ユーザーの氏名、住所、メールアドレスなどの個人情報を収集することはありません。

■ 2. 外部送信について
本アプリは、ユーザーの個人情報を外部へ送信することはありません。

ただし、本アプリはアップデート確認や関連情報の取得のために GitHub へアクセスする場合があります。
この通信により、IPアドレスやユーザーエージェントなどの技術的情報が、GitHubのサーバーへ送信される可能性があります。

これらの情報は本アプリによって収集・保存されることはありません。

■ 3. 利用目的
外部通信は、以下の目的に限り行われます。
・本アプリの最新バージョンの確認
・関連情報（公式サイトURL等）の取得

■ 4. 外部サービス
本アプリは、以下の外部サービスを利用する場合があります。
・GitHub

外部サービスにおける情報の取り扱いについては、各サービスのプライバシーポリシーをご確認ください。

■ 5. システムへの影響
本アプリは、レジストリの変更など、システム設定に影響を与える処理は行いません。

■ 6. 免責事項
本アプリの利用により生じたいかなる損害についても、作者は責任を負いません。

■ 7. 改定
本ポリシーは、必要に応じて予告なく変更される場合があります。

■ 8. お問い合わせ
本ポリシーに関するお問い合わせは、以下の連絡先までお願いいたします。
https://sites.google.com/view/gyonisostudio/contact

--------------------------------------------------------------------------

# Privacy Policy
Last Updated: 2026/03/19

This application ("the App") respects your privacy and handles information as follows:

1. Information Collection
The App does not collect any personal information such as name, address, or email address.

2. Data Transmission
The App does not transmit any personal information to external parties.

However, the App may access GitHub to check for updates or retrieve related information.
As part of this communication, technical information such as IP address and user agent may be transmitted to GitHub servers.

Such information is not collected or stored by the App.

3. Purpose of Use
External communication is performed only for the following purposes:
- Checking for the latest version of the App
- Retrieving related information (e.g., official website URL)

4. Third-Party Services
The App may use the following external service:
- GitHub

Please refer to the privacy policy of each service for details on how information is handled.

5. System Impact
The App does not modify the system registry or perform actions that affect system settings.

6. Disclaimer
The author shall not be held responsible for any damages arising from the use of the App.

7. Changes
This policy may be updated without prior notice.

8. Contact
For inquiries regarding this policy, please contact:
https://sites.google.com/view/gyonisostudio/contact
